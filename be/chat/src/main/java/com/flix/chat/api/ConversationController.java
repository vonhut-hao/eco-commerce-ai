...
public class ConversationController {
    ...
    // Đã xóa hàm trả về Entity và giữ lại hàm trả về DTO Response
    @GetMapping("/user/{userId}")
    @ResponseStatus(HttpStatus.OK)
    public ApiResponse<List<ConversationResponse>> getUserConversations(@PathVariable("userId") Long userId) {
        List<ConversationResponse> responses = conversationService.getUserConversations(userId);
        return ApiResponse.success(responses, HttpStatus.OK, "List room chat fetched successfully");
    }
